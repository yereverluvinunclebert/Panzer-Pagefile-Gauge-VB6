...
Public Function fLicenceState() As Integer
    ...
    If fFExists(gblSettingsFile) Then ' does the tool's own settings.ini exist?
        slicence = fGetINISetting("Software\PzPagefileGauge", "licence", gblSettingsFile)
...
Public Sub mnuSupport_ClickEvent()
    ...
    If answer = vbYes Then
        Call ShellExecute(menuForm.hWnd, "Open", "https://github.com/yereverluvinunclebert/Panzer-Pagefile-Gauge-" & gblCodingEnvironment & "/issues", vbNullString, App.Path, 1)
    End If
...
Public Sub readPrefsPosition()
    ...
    If gblDpiAwareness = "1" Then
        gblPrefsHighDpiXPosTwips = fGetINISetting("Software\PzPagefileGauge", "formHighDpiXPosTwips", gblSettingsFile)
        gblPrefsHighDpiYPosTwips = fGetINISetting("Software\PzPagefileGauge", "formHighDpiYPosTwips", gblSettingsFile)
    ...
    Else
        gblPrefsLowDpiXPosTwips = fGetINISetting("Software\PzPagefileGauge", "formLowDpiXPosTwips", gblSettingsFile)
        gblPrefsLowDpiYPosTwips = fGetINISetting("Software\PzPagefileGauge", "formLowDpiYPosTwips", gblSettingsFile)
...
Public Sub writePrefsPositionAndSize()
    ...
    If widgetPrefs.IsLoaded = True And widgetPrefs.WindowState = vbNormal Then ' when vbMinimised the value = -48000  !
        If gblDpiAwareness = "1" Then
            ...
            sPutINISetting "Software\PzPagefileGauge", "formHighDpiXPosTwips", gblPrefsHighDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzPagefileGauge", "formHighDpiYPosTwips", gblPrefsHighDpiYPosTwips, gblSettingsFile
        Else
            gblPrefsLowDpiXPosTwips = CStr(widgetPrefs.Left)
            gblPrefsLowDpiYPosTwips = CStr(widgetPrefs.Top)
            
            ' now write those params to the toolSettings.ini
            sPutINISetting "Software\PzPagefileGauge", "formLowDpiXPosTwips", gblPrefsLowDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzPagefileGauge", "formLowDpiYPosTwips", gblPrefsLowDpiYPosTwips, gblSettingsFile
...
Private Sub settingsTimer_Timer()
    ...
    gblUnhide = fGetINISetting("Software\PzPagefileGauge", "unhide", gblSettingsFile)

    If gblUnhide = "true" Then
        'overlayWidget.Hidden = False
        fGauge.gaugeForm.Visible = True
        sPutINISetting "Software\PzPagefileGauge", "unhide", vbNullString, gblSettingsFile
    End If
...
Public Sub hardRestart()
    ...
    If fFExists(thisCommand) Then
        ...
        Call ShellExecute(widgetPrefs.hWnd, "open", thisCommand, "Panzer-Pagefile-Gauge-VB6.exe prefs", "", 1)
    Else
