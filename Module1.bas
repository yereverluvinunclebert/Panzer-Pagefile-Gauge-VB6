...
Public Function fLicenceState() As Integer
    ...
    If fFExists(PzGSettingsFile) Then ' does the tool's own settings.ini exist?
        slicence = fGetINISetting("Software\PzPagefileGauge", "licence", PzGSettingsFile)
...
Public Sub mnuSupport_ClickEvent()
    ...
    If answer = vbYes Then
        Call ShellExecute(menuForm.hwnd, "Open", "https://github.com/yereverluvinunclebert/Panzer-Pagefile-Gauge-VB6/issues", vbNullString, App.path, 1)
    End If
...
Public Sub readPrefsPosition()
   ...
   On Error GoTo readPrefsPosition_Error
    ...
    If PzGDpiAwareness = "1" Then
        PzGFormHighDpiXPosTwips = fGetINISetting("Software\PzPagefileGauge", "formHighDpiXPosTwips", PzGSettingsFile)
        PzGFormHighDpiYPosTwips = fGetINISetting("Software\PzPagefileGauge", "formHighDpiYPosTwips", PzGSettingsFile)
...
'        End If
    Else
        PzGFormLowDpiXPosTwips = fGetINISetting("Software\PzPagefileGauge", "formLowDpiXPosTwips", PzGSettingsFile)
        PzGFormLowDpiYPosTwips = fGetINISetting("Software\PzPagefileGauge", "formLowDpiYPosTwips", PzGSettingsFile)
   ...
   On Error GoTo writePrefsPosition_Error
    ...
    If panzerPrefs.WindowState = vbNormal Then ' when vbMinimised the value = -48000  !
        If PzGDpiAwareness = "1" Then
            ...
            sPutINISetting "Software\PzPagefileGauge", "formHighDpiXPosTwips", PzGFormHighDpiXPosTwips, PzGSettingsFile
            sPutINISetting "Software\PzPagefileGauge", "formHighDpiYPosTwips", PzGFormHighDpiYPosTwips, PzGSettingsFile
        Else
            PzGFormLowDpiXPosTwips = Str$(panzerPrefs.Left)
            PzGFormLowDpiYPosTwips = Str$(panzerPrefs.Top)
            
            ' now write those params to the toolSettings.ini
            sPutINISetting "Software\PzPagefileGauge", "formLowDpiXPosTwips", PzGFormLowDpiXPosTwips, PzGSettingsFile
            sPutINISetting "Software\PzPagefileGauge", "formLowDpiYPosTwips", PzGFormLowDpiYPosTwips, PzGSettingsFile
...
Public Sub hardRestart()
    ...
    If fFExists(thisCommand) Then
        ...
        Call ShellExecute(panzerPrefs.hwnd, "open", thisCommand, "Panzer Pagefile Gauge.exe prefs", "", 1)
    Else
